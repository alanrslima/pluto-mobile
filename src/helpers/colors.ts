export class ColorsHelper {
  static newShade(color: string, percent: number) {
  ...
  static invertColor(hex: string, bw: boolean) {
    ...
    return (
      '#' +
      ColorsHelper.padZero(r) +
      ColorsHelper.padZero(g) +
      ColorsHelper.padZero(b)
    );
  }

  static padZero(str: string, len = 2) {
    var zeros = new Array(len).join('0');
    return (zeros + str).slice(-len);
  }

  static invertHex(hex: string) {
